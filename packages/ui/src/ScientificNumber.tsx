...
export function ScientificNumber({ value, precision = 4, className = "", suffix, prefix, multiLine = false }: ScientificNumberProps) {
  ...
  if (absValue === 0 || (absValue >= 0.001 && absValue < 1000000)) {
    ...
    return (
      <span className={className}>
        {prefix}{formattedStr} {suffix && <span className="text-[0.4em] text-black dark:text-white font-medium ml-3 uppercase">{suffix}</span>}
      </span>
    );
  }
  ...
  return (
    <span className={`inline-flex items-baseline ${className}`}>
      ...
      {suffix && <span className="text-[0.5em] text-black dark:text-white font-medium ml-3 uppercase">{suffix}</span>}
    </span>
  );
}
