...
enum CalculatorMode { standard, scientific, graphPlotter, astroPhysics, scienceEquation }
...
class CalculatorController extends Notifier<CalculatorState> {
  ...
  void setMode(CalculatorMode mode) {
    state = state.copyWith(mode: mode, expression: '', result: '0', justEvaluated: false);
  }
  ...
  void clearEntry() {
    ...
    if (match != null) {
      state = state.copyWith(
        expression: state.expression.substring(0, state.expression.length - match.group(0)!.length)
      );
    } else {
  ...
  void _evaluate() {
    ...
    try {
      ...
      if (state.mode != CalculatorMode.standard) {
        ...
        final sumRegex = RegExp(r'\\sum_\{([a-zA-Z])=([0-9]+)\}\^\{([0-9]+)\}\(([^)]+)\)');
  ...
  void evaluateFinal() {
    _evaluate();
    if (state.result.isNotEmpty && state.expression != state.result && state.expression.isNotEmpty) {
      List<String> newHistory = List.from(state.history)..insert(0, '${state.expression} = ${state.result}');
  ...
  void memoryAdd() {
    double currentVal = double.tryParse(state.result.replaceAll(',', '')) ?? 0.0;
    if (state.result.isEmpty) currentVal = double.tryParse(state.expression.replaceAll(',', '')) ?? 0.0;
  ...
  void memoryRecall() {
    ...
    if (state.justEvaluated) {
       state = state.copyWith(expression: memStr, result: '0', justEvaluated: false);
    } else {
  ...
  void applyPercentage() {
    if (state.expression.isEmpty || state.justEvaluated) {
      ...
      state = state.copyWith(result: newStr, expression: newStr, justEvaluated: true);
      return;
    }
    ...
    if (match != null) {
      ...
      final prefixStr = state.expression.substring(0, state.expression.length - lastOperandStr.length);
      
      // Evaluate prefix if it ends with an operator (e.g. "50 + ")
      if (prefixStr.trim().isNotEmpty && RegExp(r'[+−×÷*/]$').hasMatch(prefixStr.trim())) {
         // Create a temporary state to evaluate prefix
         String cleanPrefix = prefixStr.trim();
         cleanPrefix = cleanPrefix.substring(0, cleanPrefix.length - 1); // remove trailing operator
         
         double prefixResult = 0;
         if (cleanPrefix.isNotEmpty) {
            try {
              String expStr = cleanPrefix.replaceAll('×', '*').replaceAll('÷', '/').replaceAll('−', '-');
              Parser p = Parser();
              Expression exp = p.parse(expStr);
  ...
  void toggleSign() {
    if (state.justEvaluated) {
      if (state.result.startsWith('-')) {
        state = state.copyWith(expression: state.result.substring(1), result: '0', justEvaluated: false);
      } else if (state.result != '0') {
        state = state.copyWith(expression: '-${state.result}', result: '0', justEvaluated: false);
      }
      _evaluate();
      return;
    }
    ...
    if (match != null) {
      ...
      final prefix = state.expression.substring(0, state.expression.length - lastOperand.length);
      ...
      if (state.expression.isEmpty || RegExp(r'[+−×÷*/]$').hasMatch(state.expression.trim())) {
...
final calculatorControllerProvider = NotifierProvider<CalculatorController, CalculatorState>(CalculatorController.new);
