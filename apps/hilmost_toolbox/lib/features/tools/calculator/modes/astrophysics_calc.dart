...
enum AstroCategory { observational, theoretical }
...
class AstroEquation {
  ...
  final AstroResult Function(
    Map<String, AstroInputData> inputs,
    String outputUnit,
  )
  calculate;
...
double normalizeMass(double value, String unit) {
  switch (unit) {
    ...
    case 'Jupiter Mass':
      return value * 1.898e27;
    case 'Solar Mass':
      return value * 1.989e30;
    case 'kg':
    default:
      return value;
  }
...
double convertToTargetDistance(double baseVal, String targetUnit) {
  switch (targetUnit) {
    ...
    case 'km':
      return baseVal / 1000;
    case 'Miles':
      return baseVal / 1609.34;
    case 'Inches':
      return baseVal / 0.0254;
    case 'AU':
      return baseVal / 1.496e11;
    case 'Light Years':
      return baseVal / 9.461e15;
    case 'Parsecs':
      return baseVal / 3.086e16;
    case 'm':
    default:
      return baseVal;
  }
...
double convertToTargetTime(double baseVal, String targetUnit) {
  switch (targetUnit) {
    case 'Years':
      return baseVal / (365.25 * 24 * 3600);
    case 'Days':
      return baseVal / (24 * 3600);
    case 'Seconds':
    default:
      return baseVal;
  }
...
final AstroVariable vMass1 = AstroVariable(
  id: 'm1',
  name: 'Mass',
  hint: 'e.g. 5.97e24',
  units: massUnits,
...
final AstroVariable vMass2 = AstroVariable(
  id: 'm2',
  name: 'Mass 2',
  hint: 'e.g. 7.34e22',
  units: massUnits,
...
final AstroVariable vRadius = AstroVariable(
  id: 'r',
...
final AstroVariable vVelocity = AstroVariable(
  id: 'v',
  name: 'Velocity',
  hint: 'e.g. 299792',
  units: velocityUnits,
...
final AstroVariable vTemp = AstroVariable(
  id: 't',
  name: 'Temperature',
  hint: 'e.g. 5778',
  units: tempUnits,
...
final AstroVariable vLum = AstroVariable(
  id: 'l',
  name: 'Luminosity',
  hint: 'e.g. 1.0',
  units: lumUnits,
...
final AstroVariable vAperture = AstroVariable(
  id: 'ap',
  name: 'Aperture',
  hint: 'e.g. 200',
  units: lengthMmUnits,
...
final AstroVariable vFocalScope = AstroVariable(
  id: 'fs',
...
final AstroVariable vFocalEye = AstroVariable(
  id: 'fe',
...
final AstroVariable vApparentFov = AstroVariable(
  id: 'afov',
  name: 'Apparent FOV',
  hint: 'e.g. 50',
  units: ['degrees'],
...
final AstroVariable vRedshift = AstroVariable(
  id: 'z',
  name: 'Redshift (z)',
  hint: 'e.g. 0.1',
  units: redshiftUnits,
...
final AstroVariable vHubble = AstroVariable(
  id: 'h0',
  name: 'Hubble Constant (H₀)',
  hint: 'e.g. 70',
  units: hubbleUnits,
  defaultUnit: 'km/s/Mpc',
);
final AstroVariable vSemiMajor = AstroVariable(
  id: 'a',
...
final AstroVariable vOmegaM = AstroVariable(
  id: 'om',
  name: 'Matter Density (Ωm)',
  hint: 'e.g. 0.3',
  units: ['None'],
  defaultUnit: 'None',
);
final AstroVariable vOmegaL = AstroVariable(
  id: 'ol',
  name: 'Dark Energy (ΩΛ)',
  hint: 'e.g. 0.7',
  units: ['None'],
...
final AstroVariable vWavelength = AstroVariable(
  id: 'wl',
  name: 'Wavelength (λ)',
  hint: 'e.g. 550',
...
final AstroVariable vApparentMag = AstroVariable(
  id: 'm_app',
  name: 'Apparent Mag (m)',
  hint: 'e.g. 1.5',
  units: magUnits,
...
final AstroVariable vAbsMag = AstroVariable(
  id: 'M_abs',
  name: 'Absolute Mag (M)',
  hint: 'e.g. -4.8',
  units: magUnits,
...
final List<AstroEquation> astroEquations = [
  ...
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      return AstroResult(
        'Escape Velocity (v)',
        '${NumberFormatter.format(convertToTargetVelocity(v, outUnit))} $outUnit',
      );
    },
  ),
  AstroEquation(
  ...
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      return AstroResult(
        ...
        '${NumberFormatter.format(convertToTargetDistance(r, outUnit))} $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      if (v >= c)
        return const AstroResult('Error', 'Velocity >= c', isError: true);
  ...
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      final m2 = inputs.containsKey('m2')
          ? normalizeMass(inputs['m2']!.rawValue, inputs['m2']!.unit)
          : 0.0;
      ...
      final tSeconds = math.sqrt(
        (4 * math.pow(math.pi, 2) * math.pow(a, 3)) / (G * m),
      );
      return AstroResult(
        'Orbital Period (T)',
        '${NumberFormatter.format(convertToTargetTime(tSeconds, outUnit))} $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      return AstroResult(
        ...
        '${NumberFormatter.format(f)} $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      return AstroResult(
        'Surface Gravity (g)',
        '${NumberFormatter.format(g)} $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      return AstroResult(
        'ISCO Radius',
        '${NumberFormatter.format(convertToTargetDistance(isco, outUnit))} $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      final m = normalizeMass(inputs['m1']!.rawValue, inputs['m1']!.unit);
      if (m == 0)
        return const AstroResult('Error', 'Mass cannot be zero', isError: true);
      final tK = 1.227e23 / m;
      final temp = outUnit == 'Celsius' ? tK - 273.15 : tK;
      return AstroResult(
        'Hawking Temperature (T_H)',
        '${NumberFormatter.format(temp)} $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      return AstroResult(
        'Luminosity (L)',
        '${NumberFormatter.format(convertToTargetLuminosity(lWatts, outUnit))} $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      final t = normalizeTemp(inputs['t']!.rawValue, inputs['t']!.unit);
      if (t == 0)
        return const AstroResult('Error', 'Temp cannot be zero', isError: true);
      final wMeters = wienB / t;
      return AstroResult(
        'Peak Wavelength (λ_max)',
        '${NumberFormatter.format(convertToTargetWavelength(wMeters, outUnit))} $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      final om = inputs.containsKey('om') && inputs['om']!.rawValue > 0
          ? inputs['om']!.rawValue
          : 0.3;
      final ol = inputs.containsKey('ol') && inputs['ol']!.rawValue > 0
          ? inputs['ol']!.rawValue
          : 0.7;
      if (h0 == 0)
        return const AstroResult('Error', 'H0 cannot be zero', isError: true);
      ...
      String cDist = NumberFormatter.format(
        convertToTargetDistance(dMetersComoving, outUnit),
      );
      String lDist = NumberFormatter.format(
        convertToTargetDistance(dMetersLum, outUnit),
      );
      return AstroResult(
        'Comoving | Luminosity',
        '$cDist $outUnit | $lDist $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      if (l <= 0)
        return const AstroResult('Error', 'Luminosity > 0', isError: true);
      final mag = 4.83 - 2.5 * (math.log(l) / math.ln10);
      return AstroResult(
        'Absolute Magnitude (M)',
        '${NumberFormatter.format(mag)} $outUnit',
      );
    },
  ),
  ...
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      final d =
          normalizeDistance(inputs['ap']!.rawValue, inputs['ap']!.unit) * 1000;
      if (d == 0)
        return const AstroResult(
          'Error',
          'Aperture cannot be zero',
          isError: true,
        );
      final r = 116 / d;
      return AstroResult(
        'Resolving Power',
        '${NumberFormatter.format(r)} $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      return AstroResult(
        'Resolving Power',
        '${NumberFormatter.format(thetaArcsec)} $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      final dMeters = normalizeDistance(
        inputs['r']!.rawValue,
        inputs['r']!.unit,
      );
      ...
      return AstroResult(
        ...
        '${NumberFormatter.format(appMag)} $outUnit',
      );
    },
  ),
  AstroEquation(
    ...
    calculate: (inputs, outUnit) {
      ...
      final fe = normalizeDistance(inputs['fe']!.rawValue, inputs['fe']!.unit);
      if (fe == 0 || fs == 0)
        return const AstroResult('Error', 'Focal Lengths > 0', isError: true);
      final mag = fs / fe;
      final tfov = afov / mag;
      return AstroResult(
        'True FOV',
        '${NumberFormatter.format(tfov)} $outUnit',
      );
    },
  ),
];
...
class _AstroPhysicsCalcState extends State<AstroPhysicsCalc> {
  ...
  late AstroEquation _selectedEquation = astroEquations.first;
  int _targetVarIndex = 0; // The variable we are solving for
  ...
  Offset _historyWindowPos = const Offset(20, 20);
    ...
    if (allFilled) {
      try {
        ...
        setState(() {
          if (res.value == 'NaN' ||
              res.value == 'Infinity' ||
              res.value.contains('NaN') ||
              res.value.contains('Infinity')) {
            _result = AstroResult(
              res.label,
              'Math Error: Division by zero or invalid input',
              isError: true,
            );
          } else {
            _result = res;
  ...
  void _triggerAutoSave() {
    _historyDebounceTimer?.cancel();
    _historyDebounceTimer = Timer(const Duration(milliseconds: 1500), () {
  ...
  void _saveToHistory({bool showSnackbar = true}) {
    if (_result != null && !_result!.isError) {
      final newEntry =
          '${_selectedEquation.name}: ${_result!.label} = ${_result!.value}';
      if (_history.isNotEmpty && _history.first == newEntry) return;
  ...
  Widget _buildHistoryWindow() {
    return Positioned(
      left: _historyWindowPos.dx,
      top: _historyWindowPos.dy,
      child: GestureDetector(
        onPanUpdate: (details) {
          setState(() {
            _historyWindowPos += details.delta;
          });
        },
        child: Material(
          elevation: 8,
                    ...
                    child: Row(
                      ...
                      children: [
                        TextButton.icon(
                          ...
                          onPressed: () {
                            Clipboard.setData(
                              ClipboardData(text: _history.join('\n')),
                            );
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('History copied!')),
                            );
                          },
                        ),
                        TextButton.icon(
  ...
  String _getUnitType(String text) {
    ...
    if (lower.contains('speed') ||
        lower.contains('vel') ||
        (lower.contains('(m/s)') && !lower.contains('m/s²')))
      return 'velocity';
    if (lower.contains('accel') || lower.contains('(m/s²)'))
      return 'acceleration';
    ...
    return Builder(
      builder: (context) {
        ...
        if (_activeFocusId.isNotEmpty) {
          final activeVar = _selectedEquation.inputs
              .cast<AstroVariable?>()
              .firstWhere((v) => v?.id == _activeFocusId, orElse: () => null);
          if (activeVar != null) {
        ...
        return DropdownButtonFormField<String>(
          ...
          items: [
            const DropdownMenuItem(
              value: '',
              child: Text('Select a value...', overflow: TextOverflow.ellipsis),
            ),
            ...filteredConstants.map(
              (e) => DropdownMenuItem(
                value: e.key,
                child: Text(
                  '${e.key} = ${e.value}',
            ...
            child: TextField(
              ...
              onTap: () {
                ...
                Future.delayed(const Duration(milliseconds: 100), () {
                  final node = _focusNodes[variable.id];
          ...
          if (variable.units.length > 1)
            Expanded(
              ...
              child: DropdownButtonFormField<String>(
                ...
                onChanged: (val) {
                  setState(() {
                    _selectedUnits[variable.id] = val!;
                    _calculate();
                  });
                },
              ),
  ...
  Widget build(BuildContext context) {
    final currentEqs = astroEquations
        ...
        Row(
          ...
          children: [
            SegmentedButton<AstroCategory>(
              segments: const [
                ButtonSegment(
                  value: AstroCategory.theoretical,
                  label: Text('Theory & Cosmology'),
                ),
                ButtonSegment(
                  value: AstroCategory.observational,
                  label: Text('Observational'),
                ),
              ],
              ...
              children: [
                ...
                Expanded(
                  ...
                  child: DropdownButtonFormField<String>(
                    ...
                    isExpanded: true,
                    items: _selectedEquation.outputUnits
                        .map(
                          (u) => DropdownMenuItem(
                            value: u,
                            child: Text(u, overflow: TextOverflow.ellipsis),
                  ...
                  Flexible(
                    child: Text(
                      ...
                      style: TextStyle(
                        ...
                        fontWeight: _result != null
                            ? FontWeight.bold
                            : FontWeight.normal,
                        color: (_result?.isError ?? false)
                            ? Theme.of(context).colorScheme.error
                            : Theme.of(context).colorScheme.onPrimaryContainer
                                  .withAlpha(_result == null ? 150 : 255),
                      ),
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (_isHistoryOpen) {
          return Stack(children: [content, _buildHistoryWindow()]);
        }
        return content;
      },
    );
