...
class KeyboardMappingEngine {
  ...
  String? mapKeyEvent(KeyEvent event, {Map<String, String>? customOverrides}) {
    if (event is KeyDownEvent || event is KeyUpEvent) {
      ...
      // 1. Check character first to support shifted symbols like '+' or '*' properly
      if (char != null) {
        if (customOverrides != null && customOverrides.containsKey(char)) {
          return customOverrides[char];
        }
        if (defaultLabelMap.containsKey(char)) {
          return defaultLabelMap[char];
        }
      }

      // 2. Check exact logical key mapping (e.g. Numpad or structural keys)
      if (defaultKeyMap.containsKey(key)) {
        return defaultKeyMap[key];
      }

      // 3. Check label mapping (character keys fallback)
