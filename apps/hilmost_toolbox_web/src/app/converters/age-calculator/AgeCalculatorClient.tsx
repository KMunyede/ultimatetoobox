...
export function AgeCalculatorClient() {
  ...
  const results = useMemo(() => {
    ...
    return { years: 0, months: 0, days: 0, totalDays: 0, totalWeeks: 0, totalHours: 0, totalMinutes: 0 };
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
        {/* Form */}
        <div className="md:col-span-3 bg-white dark:bg-slate-900 border border-[var(--color-border-base)] dark:border-slate-800 rounded-2xl p-4 md:p-5 space-y-8">
        ...
        {/* Results */}
        <div id="tour-age-results" className="md:col-span-2 bg-white dark:bg-slate-900 border border-[var(--color-border-base)] dark:border-slate-800 rounded-2xl p-4 md:p-5 flex flex-col justify-center relative overflow-hidden">
            ...
            <div className="relative z-10 text-center space-y-4 py-4">
                ...
                <div className="flex items-baseline justify-center gap-2">
                    <span className="text-4xl md:text-5xl font-normal text-brand-primary tracking-tighter tabular-nums">{results.years}</span>
            ...
            <div className="relative z-10 grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-slate-100 dark:border-slate-800">
                ...
                <div className="text-center space-y-1">
                    <span className="text-caption font-normal uppercase tracking-widest text-black dark:text-white">Total Hours</span>
                    <p className="text-2xl font-normal text-black dark:text-white tabular-nums">{results.totalHours.toLocaleString()}</p>
                </div>
                <div className="text-center space-y-1">
                    <span className="text-caption font-normal uppercase tracking-widest text-black dark:text-white">Total Minutes</span>
                    <p className="text-2xl font-normal text-black dark:text-white tabular-nums">{results.totalMinutes.toLocaleString()}</p>
                </div>
            </div>
        </div>
      </div>
