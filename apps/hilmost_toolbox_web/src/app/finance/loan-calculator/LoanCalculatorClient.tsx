  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Input Form */}
        <div className="bg-white dark:bg-slate-900 border-2 border-[var(--color-border-base)] dark:border-slate-800 rounded-3xl p-5 md:p-6 space-y-5 shadow-sm">
          <NumberInput
            label="Loan Amount ($)"
            value={amount}
        ...
        {/* Results */}
        <div id="tour-loan-results" className="bg-white dark:bg-slate-900 border-2 border-[var(--color-border-base)] dark:border-slate-800 rounded-3xl p-5 md:p-6 flex flex-col justify-between shadow-sm relative overflow-hidden">
          ...
          <div className="relative z-10 grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-[var(--color-border-base)] dark:border-slate-800">
      ...
      {schedule.length > 0 && (
        <div className="bg-white dark:bg-slate-900 border-2 border-[var(--color-border-base)] dark:border-slate-800 rounded-[2.5rem] p-5 md:p-6 shadow-sm space-y-5">
            ...
            <div className="max-h-[500px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800">
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 bg-slate-50 dark:bg-slate-800 z-10 border-b border-[var(--color-border-base)] dark:border-slate-700">
                  <tr>
                    <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Month</th>
                    <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Payment</th>
                    <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Principal</th>
                    <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Interest</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {schedule.map((row) => (
                    <tr key={row.month} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="p-4 text-xs font-bold text-slate-400">#{row.month}</td>
                      <td className="p-4 text-xs font-black text-slate-900 dark:text-white">{formatCurrency(row.payment)}</td>
                      <td className="p-4 text-xs font-bold text-emerald-600">{formatCurrency(row.principal)}</td>
                      <td className="p-4 text-xs font-bold text-rose-500">{formatCurrency(row.interest)}</td>
                      <td className="p-4 text-xs font-black text-slate-900 dark:text-white text-right">{formatCurrency(row.balance)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
