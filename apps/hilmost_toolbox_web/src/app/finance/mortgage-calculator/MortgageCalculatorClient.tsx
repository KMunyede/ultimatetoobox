        ...
        <div className="bg-white dark:bg-slate-900 border-2 border-[var(--color-border-base)] dark:border-slate-800 rounded-3xl p-4 md:p-5 flex flex-col justify-between shadow-sm relative overflow-hidden">
          ...
          <div className="relative z-10 grid grid-cols-3 gap-2 mt-4 pt-4 border-t border-[var(--color-border-base)] dark:border-slate-800">
            <div className="text-center space-y-1">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">P&I</span>
                <p className="text-sm font-black text-slate-900 dark:text-white">{formatCurrency(monthlyPI)}</p>
            </div>
            <div className="text-center space-y-1">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Tax & Ins.</span>
                <p className="text-sm font-black text-slate-900 dark:text-white">{formatCurrency(monthlyTax + monthlyInsurance)}</p>
            </div>
            <div className="text-center space-y-1">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Total Interest</span>
                <p className="text-sm font-black text-slate-900 dark:text-white">{formatCurrency(totalInterest)}</p>
            </div>
          </div>
        </div>
      </div>
