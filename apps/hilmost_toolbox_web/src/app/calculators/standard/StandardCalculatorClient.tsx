...
export function StandardCalculatorClient() {
  ...
  return (
    <motion.div 
      ...
      className="@container grid grid-cols-1 lg:grid-cols-3 gap-5 max-w-6xl mx-auto"
    >
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-canvas-card border border-border-base rounded-[3rem] p-5 md:p-8 shadow-2xl relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-1.5 bg-orange-500/20" />

          {/* Display */}
          <div id="tour-standard-display" className="bg-canvas-muted border border-border-base rounded-2xl p-5 mb-8 text-right h-44 flex flex-col justify-end shadow-inner relative group">
            <div className="absolute top-4 left-6 flex items-center gap-2">
               <div className="w-2 h-2 rounded-full bg-orange-500 animate-pulse" />
               <span className="text-xs font-black text-text-muted uppercase tracking-[0.2em]">Standard Engine</span>
            </div>
            {parseFloat(memory) !== 0 && (
              <div className="absolute top-4 right-6 text-xs font-black text-brand-primary uppercase tracking-widest bg-brand-primary/10 px-2 py-0.5 rounded">M</div>
            )}
          ...
          <div id="tour-standard-keypad" className="grid grid-cols-4 gap-4">
            {/* Row 1 */}
            <button onClick={clear} className="h-14 bg-red-500/10 text-red-600 font-black rounded-2xl border border-red-500/10 hover:bg-red-500/20 transition-all text-lg">AC</button>
            <button onClick={backspace} className="h-14 bg-canvas-muted text-text-secondary font-black rounded-2xl border border-border-base hover:border-text-muted transition-all flex items-center justify-center"><Delete size={20} /></button>
            <button onClick={handlePercent} className="h-14 bg-orange-500/10 text-orange-600 font-black rounded-2xl border border-orange-500/10 hover:bg-orange-500/20 transition-all text-xl">%</button>
            <button onClick={() => handleOp("÷")} className={`h-14 font-black rounded-2xl transition-all text-2xl border ${op === '÷' && waitingForNewVal ? 'bg-orange-500 text-white border-orange-500 shadow-lg shadow-orange-500/20' : 'bg-orange-500/10 text-orange-600 border-orange-500/10 hover:bg-orange-500/20'}`}>÷</button>

            {/* Row 2 (7-8-9) */}
            {[7,8,9].map(n => (
              <button key={n} onClick={() => handleNum(String(n))} className="h-14 bg-canvas-card text-text-primary font-black text-2xl rounded-2xl border border-border-base hover:bg-canvas-muted transition-all shadow-sm">{n}</button>
            ))}
            <button onClick={() => handleOp("×")} className={`h-14 font-black rounded-2xl transition-all text-2xl border ${op === '×' && waitingForNewVal ? 'bg-orange-500 text-white border-orange-500 shadow-lg shadow-orange-500/20' : 'bg-orange-500/10 text-orange-600 border-orange-500/10 hover:bg-orange-500/20'}`}>×</button>
            ...
            {/* Row 5 */}
            <button onClick={useAns} className="h-14 bg-canvas-muted text-brand-primary font-black text-sm rounded-2xl border border-border-base hover:border-brand-primary/50 transition-all uppercase tracking-tighter">Ans</button>
            <button onClick={() => handleNum("0")} className="h-14 bg-canvas-card text-text-primary font-black text-2xl rounded-2xl border border-border-base hover:bg-canvas-muted transition-all shadow-sm">0</button>
            <button onClick={() => handleNum(".")} className="h-14 bg-canvas-card text-text-primary font-black text-2xl rounded-2xl border border-border-base hover:bg-canvas-muted transition-all shadow-sm">.</button>
            <button onClick={handleEqual} className="h-14 bg-brand-primary text-white font-black text-3xl rounded-2xl border border-brand-primary hover:bg-brand-hover transition-all shadow-2xl shadow-brand-primary/30">=</button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
          <div className="bg-canvas-card border border-border-base rounded-[2.5rem] p-5 shadow-xl h-full min-h-[400px]">
                ...
                <AnimatePresence mode="popLayout">
                  {history.length > 0 ? (
                    history.map((entry, i) => (
                      ...
                      >
                        <p className="text-xs font-black text-text-muted uppercase tracking-widest mb-1 group-hover:text-brand-primary">Calculation {history.length - i}</p>
